...
本编译器主要参考的编译器为 Pascal 编译器。

### 总体结构

该编译器总体结构由经典的词法分析、语法分析、错误处理、语义分析、代码生成和代码优化组成。

### 接口设计

该编译器设计了较多的接口，故选取几个重要的接口进行介绍如下：

1. `nextch`：读取源程序的下一个字符。
2. `error`：接收错误编号作为输入，功能是打印错误位置和错误编号，并将错误编号加入到错误集合 `errs` 中。
3. `enter`：接收标识符信息作为输入，负责将标准类型、过程、函数名登记到符号表中。
4. `emit` 系列接口：根据没有操作数、一个操作数、两个操作数生成对应的 PCODE 指令，这系列接口都需要一个输入 `fct` 表示操作码，根据具体接口功能需要另外若干个操作数作为输入，以生成对应的 PCODE 指令。
5. `statement`：对各种语句进行分析处理。

### 文件组织

课程组提供的参考编译器代码均放在了一个文件中，不知道原始代码是否也是这样。个人认为原有的实现应该是将各个模块的代码分别放在不同的文件中，这样更符合低耦合的设计原则。
